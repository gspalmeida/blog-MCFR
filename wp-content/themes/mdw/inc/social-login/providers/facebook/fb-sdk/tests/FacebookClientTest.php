...
class MyFooBatchClientHandler implements FacebookHttpClientInterface {
	...
	public function send( $url, $method, $body, array $headers, $timeOut ) {
		return new GraphRawResponse(
		"HTTP/1.1 200 OK\r\nDate: Mon, 19 May 2014 18:37:17 GMT", '[{"code":"123","body":"Foo"},{"code":"1337","body":"Bar"}]'
		);
	}
...
class FacebookClientTest extends \PHPUnit_Framework_TestCase {
	...
	public function testCanCreateATestUserAndGetTheProfileAndThenDeleteTheTestUser() {
		...
		static::$testFacebookApp, static::$testFacebookApp->getAccessToken(), 'POST', $testUserPath, $params
		);
		...
		$request	 = new FacebookRequest(
		static::$testFacebookApp, $testUserAccessToken, 'GET', '/me'
		);
		...
		// Delete test user
		$request	 = new FacebookRequest(
		static::$testFacebookApp, static::$testFacebookApp->getAccessToken(), 'DELETE', '/' . $testUserId
		);
		$graphNode	 = static::$testFacebookClient->sendRequest( $request )->getGraphNode();
