...
add_action( 'widgets_init', function() {
	register_widget( 'MDW_Divider' );
} );

/**
 * Divider widget.
 */
class MDW_Divider extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
		'mdw_divider', // Base ID
  __( 'MDW Divider', 'mdw' ), // Name
	  array( 'description'	 => __( 'MDW Divider', 'mdw' ),
		...
		if ( get_the_ID() == $page_id || $page_id == 'All pages' ) {
			...
			for ( $i = 1; $i <= 1; $i++ ) {
				...
				if ( $template_number == $i ) {
					...
					if ( $template == '' )
						$template = dirname( __DIR__ ) . '/divider/templates/divider-template-' . $i . '.php';
				}
			}
	...
	public function form( $instance ) {
		...
		$color	 = ( isset( $instance[ "color" ] ) ) ? $instance[ "color" ] : "#32373c";
		...
		animations_dropdown( $this->get_field_name( 'animation' ), $this->get_field_id( 'animation' ), $animation );
		?>
		<div class="titlepage_widget">
			...
			<div class="tab-content">
				...
				<div class="tab-pane fade <?php echo ($template_number == 1) ? 'active in' : '' ?>" id="v1" role="tabpanel">
					...
					<!--/.Title -->
					<div class='widget_input col-md-12'>
						<h4><?php echo _e('Line color', 'mdw'); ?></h4>
						<?php $WG->insertColorPicker( $this, ${'color'}, 'color' ) ;?>
						<br>
					</div>
				</div>
				<!--/.Panel 1-->
