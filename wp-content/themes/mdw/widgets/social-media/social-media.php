...
add_action( 'widgets_init', function() {
	register_widget( 'MDW_Social_Media' );
} );

/**
 * MDW Accordion widget.
 */
class MDW_Social_Media extends WP_Widget {
  ...
  __( 'MDW Social Media', 'mdw' ), // Name
		...
		if ( get_the_ID() == $page_id || $page_id == 'All pages' ) {
			...
			for ( $i = 1; $i <= 2; $i++ ) {
				...
				if ( $template_number == $i ) {
					...
					if ( $template == '' )
						$template = dirname( __DIR__ ) . '/social-media/templates/social-media-template-' . $i . '.php';
				}
			}
		...
		<div class="titlepage_widget">
			...
			<div class="tab-content">
				...
				<div class="tab-pane fade <?php echo ($template_number == 1) ? 'active in' : '' ?>" id="v1" role="tabpanel">
					...
					<div id="custom-panel">
						<br>
						<div class="widget_input col-md-6"> <span id="add-social <?php echo $this->get_field_id( 'result' ); ?>" name="<?php echo $this->get_field_name( 'result' ); ?>-panel1"><?php _e( 'Add Social icon', 'mdw' ); ?> <i class="fa fa-plus-circle blue-text"></i></span> </div>
						<div class="widget_input col-md-6"> <span id="delete-feature" name="<?php echo $this->get_field_name( 'result' ); ?>-panel1"><?php _e( 'Delete Social icon', 'mdw' ); ?> <i class="fa fa-minus-circle red-text"></i></span> </div>
						...
						<div id="<?php echo $this->get_field_name( 'result' ); ?>-panel1">
							<?php for ( $i = 1; $i <= $fieldCount; $i++ ) { ?>							
								<h4 id='toggler' class="col-md-12" onclick='jQuery( this ).next().stop().slideToggle();'><?php _e( 'Icon', 'mdw' ); ?> <?php echo $i; ?> 
				...
				<div class="tab-pane fade <?php echo ($template_number == 2) ? 'active in' : '' ?>" id="v2" role="tabpanel">
					...
					<div class="widget_input">
						<?php $WG->textInput( 'insta', ${'insta'}, 'Instagram username', $this ) ?>
						<br/>
					</div>
					<div class="widget_input">
						<?php $WG->textInput( 'insta_url', ${'insta_url'}, 'Instagram profile url', $this ) ?>
						<br/>
					</div>       
                    <div class='widget_input'>
                        <?php $WG->numberInput( 'img_count', $img_count, 'Image amount: ', $this ) ?>
                        <br/>
                    </div>      
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance[ 'insta' ] = (!empty( $new_instance[ 'insta' ] ) ) ? ( $new_instance[ 'insta' ] ) : "";

		$instance[ 'insta_url' ] = (!empty( $new_instance[ 'insta_url' ] ) ) ? ( $new_instance[ 'insta_url' ] ) : "";

		$instance[ 'img_count' ] = (!empty( $new_instance[ 'img_count' ] ) ) ? ( $new_instance[ 'img_count' ] ) : "";
