<?php
/*
  Plugin Name: MDW Intro: Contact Form
...
add_action( 'widgets_init', function() {
	register_widget( 'MDW_Intro_Contact_Form' );
} );

/**
 * MDW_Intro_Contact_Form widget.
 */
class MDW_Intro_Contact_Form extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
		'MDW_Intro_Contact_Form', // Base ID
  __( 'MDW Intro: Contact Form', 'mdw' ), // Name
	  array( 'description'	 => __( 'Contact form intro', 'mdw' ),
	...
	public function widget( $w_args, $instance ) {
		...
		if ( get_the_ID() == $page_id || $page_id == 'All pages' ) {
			...
			wp_register_style( 'MDW-intro-contact-form-custom_styles', get_template_directory_uri() . '/widgets/css/intro.css' );
			wp_enqueue_style( 'MDW-intro-contact-form-custom_styles' );

			// use a template for the output so that it can easily be overridden by theme
			// check for template in active theme
			$template = locate_template( array( 'contact-form-template.php' ) );

			// if none found use the default template
			if ( $template == '' )
				$template = 'contact-form-template.php';
